...
## ORTHOMCL [2006-10-02] Version 1.4
...
our $BLAST_FORMAT                        = "full";  # "compact" corresponds to NCBI-BLAST's -m 8
...
our $MCL                                 = "/software/pathogen/external/bin/mcl";

# path
our $PATH_TO_ORTHOMCL                    = "/software/pathogen/external/applications/OrthoMCL_v1.4/";   # must end with "/" 
...
sub constructDirectory {
	...
	$ORTHOMCL_WORKING_DIR = (split(" ",$date))[1]."_".(split(" ",$date))[2];
	my $no=1;
	...
	$ORTHOMCL_WORKING_DIR =  getcwd(). "/orthomcl${$}/"; 
...
sub executeBLASTALL {
	...
	my $cmd = "$BLASTALL -p blastp -i $in -d $db -e $pv_cutoff -o $out -m $blast_flag{'m'} -a $BLAST_NOCPU -b 100000 -v 100000";
	#write_log("  Native BLASTALL command:\n  $BLASTALL -p blastp -i $in -d $db -e $pv_cutoff -o $out -m $blast_flag{'m'} -a $BLAST_NOCPU\n");
	write_log("  Native BLASTALL command:\n  $cmd\n");
	open TFH, ">${in}.cmd.txt";
	print TFH "${cmd}\n";
	close TFH;
	chmod 0755, "${in}.cmd.txt";
	my $bsub_cmd = "bsub -K -q basement -M 6000000 -R\"select[mem>6000] rusage[mem=6000]\"  -o $out -e $out.error ${in}.cmd.txt";
	#system ("$BLASTALL -p blastp -i $in -d $db -e $pv_cutoff -o $out -m $blast_flag{'m'} -a $BLAST_NOCPU");
	write_log("  BSUB command:\n  $bsub_cmd\n");
	system($bsub_cmd);
	#system ("bigger_blast.pl -o $out -p $db $in \"-e $pv_cutoff\" ");
	until (-s "$out"){
...
sub blast_parse {
	...
	write_log("\nParsing blast result (format $blast_flag{'format'})!\n");
